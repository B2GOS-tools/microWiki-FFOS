function BBC2HTML(S) {
	...
	function P($0, $1, $2, $3) {
	  ...
	  switch ($1) {
		case 'link':
			return '<a class="internal" href="#" data-href="'+ $2 + '">'+ $3 +'</a>';
	    ...
	    case 'img': 
	    	var d = D($2); return '<img src="'+ $3 +'"'+ (d ? ' width="'+ d[1] +'" height="'+ d[2] +'"' : '') +' alt="'+ (d ? '' : $2) +'" />';
	    /*!TODO: HTML 5 EINBINDUBG?!
	    case 'flash':
	    case 'youtube': var d = D($2)||[0, 425, 366]; return '<object type="application/x-shockwave-flash" data="'+ Y[$1] + $3 +'" width="'+ d[1] +'" height="'+ d[2] +'"><param name="movie" value="'+ Y[$1] + $3 +'" /></object>';
	    */
	    
	    case 'google':
	    case 'wikipedia': 
	    	return '<a href="'+ G[$1] + $3 +'" target="_blank">'+ $3 +'</a>';
	    ...
	    case 'b': case 'li': case 'ul': case 'p': case 'i':case 'u':case 's':case 'sup':case 'sub':case 'h1':case 'h2':case 'h3':case 'h4':case 'h5':case 'h6':case 'table':case 'tr':case 'th':case 'td': return '<'+ $1 +'>'+ $3 +'</'+ $1 +'>';
	...
	B['\\[quote(?:=([^\\]]+))?]'] = function($0, $1) {return '<div class="bb-quote">'+ ($1 ? '<b>' + $1 + ' schrieb</b>' : '<b>Zitat</b>') +':<blockquote>'}; B['\\[/quote]'] = '</blockquote></div>';
	B['\\[(hr|br)]'] = '<$1 />'; B['\\[sp]'] = '&nbsp;';
	return R(A(A(S, I), B));
}